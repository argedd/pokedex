...
export class PokemonService {
  ...
  async findOne(term: string) {
    ...
    //is number
    if (!isNaN(+term)) {
      pokemon = await this.pokemonRepository.findOne({ pokedexNumber: +term });
    }
    ...
    if (!pokemon) {
      throw new NotFoundException(
        `Pokemon with id, name or pokedex number "${term}" not found`,
      );
    }
  ...
  async remove(id: string) {
    ...
    const { deletedCount } = await this.pokemonRepository.deleteOne({
      _id: id,
    });
    if (deletedCount === 0)
